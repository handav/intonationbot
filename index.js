...
var bot = new Twit({
    consumer_key: process.env.INTONATIONBOT_CONSUMER_KEY,
    consumer_secret: process.env.INTONATIONBOT_CONSUMER_SECRET,
    access_token: process.env.INTONATIONBOT_ACCESS_TOKEN,
    access_token_secret: process.env.INTONATIONBOT_ACCESS_TOKEN_SECRET,
    timeout_ms: 60 * 1000
});

var bot_username = '@Intonation_Bot';
...
function uploadMedia(tweet, vidFn){
    ...
    bot.postMediaChunked({file_path: filePath}, function(err, data, response){
        if (err){
            ...
            console.log(data);
            tweet.text = tweet.text.split(bot_username).join(' ').trim();
            var params = {
